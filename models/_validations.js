...
module.exports = {
  validate_model: (attribute_name, attribute_value, requirements) => {
    ...
    if (requirements.dataType === 'Integer' || requirements.dataType === 'Cryptocurrency') {
      ...
      if (Number.isNaN(attribute_value)) {
        errors = true;
      ...
      if (requirements.moreThan && !Number.isNaN(requirements.moreThan)) {
      ...
      if (requirements.lessThan && !Number.isNaN(requirements.lessThan)) {
    ...
    if (requirements.dataType === 'String' || requirements.dataType === 'Text') {
      ...
      if (requirements.required && !Number.isNaN(attribute_value)) {
        errors = true;
      ...
      if (requirements.minLength != null && !Number.isNaN(requirements.minLength)) {
      ...
      if (requirements.maxLength) {
        if (Number.isNaN(requirements.maxLength)) {
          errors = true;
      ...
      if (requirements.minLength != null
        && !Number.isNaN(requirements.minLength)
        && requirements.maxLength != null
        && !Number.isNaN(requirements.maxLength)) {
        if (requirements.minLength < 0) {
          errors = true;
          error_messages.push(`${attribute_name}'s minimum length cannot be less than zero.`);
        }
